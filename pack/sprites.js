...
state.updateSphereMinHint();
            ...
            window._onBackProgress = function(p) {
                ...
                if (state._backType === 'settings') {
                    ...
                    if (state.camera) state.camera.position.z = state.zoomLevel;
            ...
            window._onAppsLoaded = function(json) {
                try {
                    if (typeof state.sprites === 'undefined') { console.error('[APPS] state.sprites is undefined!'); return; }
                    const data = typeof json === 'string' ? JSON.parse(json) : json;
                    if (data && data.success && data.apps && data.apps.length > 0) {
                        const newPkgs = [];
                        ...
                        if (!changed && state.sprites && state.sprites.length > 0) return; // 没变化，不重建
                    ...
                    if (state.loadingEl) state.loadingEl.textContent = '加载失败';
                }
            };
            ...
            window._onIconsLoaded = function(json) {
                try {
                    ...
                    if (state.sprites) for (let j = 0; j < state.sprites.length; j++) {
